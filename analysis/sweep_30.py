...
# ----------------------------------------------------------------------
full = sorted(glob.glob(f'{REPO}/google_supremacy_circuit_files/circuit_n12_*.py'))
patch = sorted(glob.glob(f'{REPO}/google_supremacy_circuit_files/circuit_patch_n12_*.py'))
rows, all_fsim_bits = [], []
print(f"{'instance':<28}{'1-F_pub':>12}{'1-F_int':>12}{'thm err':>11}{'R':>5}{'rank':>6}{'bits':>6}")
for cf in full + patch:
    name = os.path.basename(cf).replace('circuit_', '').replace('_pEFGH.py', '').replace('patch_', 'patch_')
    t0 = time.time()
    QO, steps, comp, fb = parse(cf)
    all_fsim_bits += fb
    R, bits, m2, m1 = counts_of(steps)
    p_id_sv = Statevector.from_instruction(ideal_circuit(steps)).data
    qc_c = mbqc_circuit(steps, bits, True)
    sv_c, _ = run_mbqc(qc_c, seed=1, want_sv=True)
    F_int = subsys_fid(sv_c, p_id_sv)
    amp_file = f"{REPO}/data/google_amplitudes/amplitudes_n12_m14_{name.replace('n12_m14_','')}_pEFGH.txt"
    F_pub = ''
    if os.path.exists(amp_file):
        F_pub = subsys_fid(sv_c, load_pub_amps(amp_file))
    qc_u = mbqc_circuit(steps, bits, False)
    sv_u, ubits = run_mbqc(qc_u, seed=2, want_sv=True)
    p_traj = np.abs(sv_u.reshape(2, D, order='C')).__pow__(2).sum(axis=0)
    flips, mask = track(steps, ubits)
    p_eff = np.abs(Statevector.from_instruction(ideal_circuit(steps, flips)).data)**2
    m = int(sum(int(mask[i]) << i for i in range(ND)))
    thm = float(np.max(np.abs(p_traj - p_eff[np.arange(D) ^ m])))
    rk = rank_of_A(steps, bits, R)
    rows.append(dict(name=name, F_pub=F_pub, F_int=F_int, thm=thm, R=R, rank=rk,
                     bits=bits, m2=m2, m1=m1, **comp))
    fp = f"{1-F_pub:>12.2e}" if F_pub != '' else f"{'—':>12}"
    print(f"{name:<28}{fp}{1-F_int:>12.2e}{thm:>11.1e}{R:>5}{rk:>6}{bits:>6}  ({time.time()-t0:.0f}s)")

with open('sweep_results.csv', 'w', newline='') as f:
    w = csv.DictWriter(f, fieldnames=list(rows[0].keys())); w.writeheader(); w.writerows(rows)

fullr = [r for r in rows if not r['name'].startswith('patch')]
pf = np.array([1-r['F_pub'] for r in fullr]); pi = np.array([1-r['F_int'] for r in rows])
th = np.array([r['thm'] for r in rows])
fbits = np.array(all_fsim_bits)
print("\n=== AGGREGATE ===")
print(f"published-amplitude infidelity (20 full): mean={pf.mean():.2e} min={pf.min():.2e} max={pf.max():.2e}")
print(f"internal infidelity (all 30): max={pi.max():.2e}")
print(f"theorem error (all 30): max={th.max():.2e}")
print(f"R: full mean={np.mean([r['R'] for r in fullr]):.1f}±{np.std([r['R'] for r in fullr]):.1f}  "
      f"rank(A): mean={np.mean([r['rank'] for r in fullr]):.1f}±{np.std([r['rank'] for r in fullr]):.1f}")
print(f"bits: full mean={np.mean([r['bits'] for r in fullr]):.0f}  2q-meas mean={np.mean([r['m2'] for r in fullr]):.0f}  "
      f"1q-meas mean={np.mean([r['m1'] for r in fullr]):.0f}")
print(f"per-fSim bits: min={fbits.min()} median={int(np.median(fbits))} max={fbits.max()}")
print(f"composition (full, per circuit): sx={np.mean([r['sx'] for r in fullr]):.1f} sy={np.mean([r['sy'] for r in fullr]):.1f} "
      f"sw={np.mean([r['sw'] for r in fullr]):.1f} rz_nc={np.mean([r['rz_nc'] for r in fullr]):.1f} "
      f"rz_cl={np.mean([r['rz_cl'] for r in fullr]):.1f} fsim={np.mean([r['fsim'] for r in fullr]):.1f}")
